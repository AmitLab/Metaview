...
output$downloadData <- downloadHandler(
  ...
  observeEvent(input$newValue,
    ...
    getPage1<-function() {
      return(includeHTML("lung_metacell_model.html"))
    }
    output$lung_metacell_model<-renderUI({getPage1()})
    
    getPage2<-function() {
      return(includeHTML("lung_lig_rec.html"))
    }
    output$lung_lig_rec<-renderUI({getPage2()})
